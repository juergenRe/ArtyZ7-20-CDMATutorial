...
#define DDR_BASE_ADDRESS     	0x08000000
#define DDR_BASE_WRITE_ADDRESS  0x0C000000
...
#define DDR_MAP_SIZE 0x4000000
#define DDR_MAP_MASK (DDR_MAP_SIZE - 1)

#define DDR_WRITE_OFFSET 0x4000000
...
{
	int memfd_dma;
    void *mapped_base_dma, *mapped_dev_base_dma;
    off_t dev_base_dma = CDMA_BASE_ADDRESS;

    int memfd_src;
    void *mapped_base_src, *mapped_dev_base_src;
    off_t dev_base_src = DDR_BASE_ADDRESS;

    int memfd_dst;
    void *mapped_base_dst, *mapped_dev_base_dst;
    off_t dev_base_dst = DDR_BASE_WRITE_ADDRESS;
    ...
    /*======================================================================================
     STEP 1 : Initialize the source buffer bytes with a pattern  and clear the Destination
  	...
  	for (Index = 0; Index < (BUFFER_BYTESIZE/4); Index++) {
  			SrcArray[Index] = Index;
  			//SrcArray[Index] = 0x5A5A5A5A/*Index & 0xFF*/;
  			DestArray[Index] = 0;
  	}

  	/*======================================================================================
  	STEP 2 : Map the kernel memory location starting from 0x08000000 to the User layer
  	========================================================================================*/
  	memfd_src = open("/dev/mem", O_RDWR | O_SYNC);
    if (memfd_src == -1) {
    ...
    mapped_base_src = mmap(0, DDR_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd_src, dev_base_src & ~DDR_MAP_MASK);
    if (mapped_base_src == (void *) -1) {
    ...
    // that was mapped as memory is mapped at the start of a page
     mapped_dev_base_src = mapped_base_src + (dev_base_src & DDR_MAP_MASK);

     /*======================================================================================
     STEP 3 : Copy the Data to the DDR Memory at location 0x20000000
     ========================================================================================*/
    memcpy(mapped_dev_base_src, SrcArray, (BUFFER_BYTESIZE));

    /*======================================================================================
     STEP 4 : Un-map the kernel memory from the User layer.
    ========================================================================================*/
    if (munmap(mapped_base_src, DDR_MAP_SIZE) == -1) {
    ...
    close(memfd_src);
  	...
  	========================================================================================*/
    memfd_dma = open("/dev/mem", O_RDWR | O_SYNC);
    if (memfd_dma == -1) {
    	printf("Can't open /dev/mem.\n");
    	exit(0);
    }
    ...
    // Map one page of memory into user space such that the device is in that page, but it may not
    // be at the start of the page.
    mapped_base_dma = mmap(0, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd_dma, dev_base_dma & ~MAP_MASK);
    if (mapped_base_dma == (void *) -1) {
    	  printf("Can't map the memory to user space.\n");
    	  exit(0);
    }
    ...
    // get the address of the device in user space which will be an offset from the base
    // that was mapped as memory is mapped at the start of a page
    mapped_dev_base_dma = mapped_base_dma + (dev_base_dma & MAP_MASK);

    //Reset CDMA
	do {
		ResetMask = (unsigned long )XAXICDMA_CR_RESET_MASK;
		*((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_CR_OFFSET)) = (unsigned long)ResetMask;
		/* If the reset bit is still high, then reset is not done	*/
		ResetMask = *((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_CR_OFFSET));
	...
	//enable Interrupt
	RegValue = *((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_CR_OFFSET));
	RegValue = (unsigned long)(RegValue | XAXICDMA_XR_IRQ_ALL_MASK );
	*((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_CR_OFFSET)) = (unsigned long)RegValue;

	// Checking for the Bus Idle
	RegValue = *((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_SR_OFFSET));
	...
	//Set the Source and Destination Address, as well as BTT
	*((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_SRCADDR_OFFSET)) = (unsigned long)DDR_BASE_ADDRESS;
	*((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_DSTADDR_OFFSET)) = (unsigned long)DDR_BASE_WRITE_ADDRESS;
	RegValue = (unsigned long)(BUFFER_BYTESIZE);
	*((volatile unsigned long *) (mapped_dev_base_dma + XAXICDMA_BTT_OFFSET)) = (unsigned long)RegValue;
	...
	========================================================================================*/
	if (munmap(mapped_base_dma, MAP_SIZE) == -1) {
		printf("Can't unmap memory from user space.\n");
		exit(0);
	}
    ...
    /*======================================================================================
    STEP 8 : Map the kernel memory location starting from 0x0C000000 to the User layer
    ========================================================================================*/
	memfd_dst = open("/dev/mem", O_RDWR | O_SYNC);
	if (memfd_dst == -1) {
	   printf("Can't open /dev/mem.\n");
	   exit(0);
	}
	...
	// Map one page of memory into user space such that the device is in that page, but it may not
	// be at the start of the page.
	mapped_base_dst = mmap(0, DDR_MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd_dst, dev_base_dst & ~DDR_MAP_MASK);
	if (mapped_base_dst == (void *) -1)	{
	   printf("Can't map the memory to user space.\n");
	   exit(0);
	}
	printf("Memory mapped at address %p.\n", mapped_base_dst);
	...
	// get the address of the device in user space which will be an offset from the base
	// that was mapped as memory is mapped at the start of a page
	mapped_dev_base_dst = mapped_base_dst + (dev_base_dst & DDR_MAP_MASK);

	/*======================================================================================
	STEP 9 : Copy the Data from DDR Memory location 0x0C000000 to Destination Buffer
	========================================================================================*/
	memcpy(DestArray, mapped_dev_base_dst, (BUFFER_BYTESIZE ));

	/*======================================================================================
	STEP 10 : Un-map the Kernel memory from the User layer.
	========================================================================================*/
	if (munmap(mapped_base_dst, DDR_MAP_SIZE) == -1) {
		printf("Can't unmap memory from user space.\n");
		exit(0);
	}
	close(memfd_dst);
